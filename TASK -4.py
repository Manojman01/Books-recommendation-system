...
def generate_library_graph(users, books, edges, genres):
    ...
    for book in books:
        genre = genres.get(book, 'known')  # Default to 'Unknown' if genre not specified
...
def generate_genre_distribution():
    ...
    genres = ['Fantasy', 'Historical Fiction', 'Thriller', 'Non-Fiction']
...
genres = {f'Book{i}': random.choice(['Fantasy', 'Historical Fiction', 'Thriller', 'Non-Fiction']) for i in range(1, 15)}
